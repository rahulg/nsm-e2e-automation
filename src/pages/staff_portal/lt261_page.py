...
# The LT-261 SALE DATE follows the same calendar rule as the LT-263 forms — a Sunday is
# rejected outright ("A Sunday sale date cannot be selected.") and leaves Submit disabled.
# CONFIRMED on STAGE 2026-08-10: create_lt261 passes future_date(21), which returns an ISO
# YYYY-MM-DD string; the MM/DD/YYYY datepicker parses that as UTC midnight, so in the
# America/New_York context the field renders the PREVIOUS day. 2026-08-31 (a Monday) became
# 8/30/2026 (a Sunday) and Submit never enabled. Both halves are handled here: the date is
# normalised to MM/DD/YYYY before it is filled, and validity is read back off the control so
# a rejected day rolls forward instead of surfacing as an unexplained Submit timeout.
...
class Lt261Page:
    ...
    def fill_make(self, make_text: str = "TOY"):
        ...
        An E-Stop raised on a VIN that already carries a case arrives pre-populated, and
        mat-chip-list hides its input once a chip is present — so waiting for that input
        to be visible would time out on a Make field that is in fact already filled.
        """
        make_input = self.page.locator("(//input[@role='combobox'])[2]")
        chips = self.page.locator(
            "xpath=(//input[@role='combobox'])[2]/ancestor::mat-chip-list[1]//mat-chip")
        if chips.count() and not make_input.is_visible():
            print(f"NOTE: MAKE pre-populated with '{chips.first.inner_text().strip()}' — kept")
            return
    ...
    def fill_sale_date(self, date_str: str) -> str:
        """Fill the Sale Date field, rolling forward past any day the form rejects.

        Accepts either MM/DD/YYYY or ISO YYYY-MM-DD and always fills MM/DD/YYYY — an ISO
        string reaches the datepicker as UTC and renders as the previous local day, which is
        how a Monday silently became a rejected Sunday. Returns the date actually accepted
        (>= the requested one) so a caller can assert on the real value.
        """
        ...
        for offset in range(SALE_DATE_MAX_ROLL_DAYS + 1):
            ...
            if "ng-invalid" not in (date_input.get_attribute("class") or ""):
                if offset:
                    print(f"NOTE: LT-261 SALE DATE {date_str} was rejected by the form "
                          f"(ng-invalid) — rolled forward to {candidate}")
                return candidate

        errors = self.page.locator("mat-error, .mat-error").all_text_contents()
        raise AssertionError(
            f"EXPECTED: an accepted LT-261 SALE DATE within {SALE_DATE_MAX_ROLL_DAYS} days "
            f"of {date_str} | ACTUAL: every candidate left the input ng-invalid — "
    ...
    # ========================================================================
    # Case / File number readout  (TW 27366957 · PRE-13)
    #
    # Nothing in the suite read the D-format case number before this: LT-261
    # records were identified by VIN only and post-submit identity was taken
    # from the /LT-261/<internal id>/details URL, which is NOT the case number.
    #
    # Live-confirmed on QA 2026-07-30:
    #   * details page — a single `span.first-heading` renders
    #     "D26-100037 | 2018 BOYTOY | ARU1VBW1XC96KYYUU | <status>"
    #   * listing table — a "FILE NUMBER" column (index 1) holds the same value
    # ========================================================================

    CASE_NUMBER_RE = re.compile(r"D\d{2}-\d{6}")

    def get_case_number(self, timeout: int = 30_000) -> str:
        """Read the D[YY]-nnnnnn case number off the open LT-261 details page.

        Polls, because the heading renders after the details chain resolves.
        Raises AssertionError (not a bare timeout) so a missing number reports
        as the defect it would be, rather than as a selector problem.
        """
        deadline = time.monotonic() + timeout / 1000.0
        last = ""
        while time.monotonic() < deadline:
            try:
                heading = self.page.locator("span.first-heading").first
                if heading.count():
                    last = (heading.inner_text() or "").strip()
                    m = self.CASE_NUMBER_RE.search(last)
                    if m:
                        return m.group(0)
                # fall back to anywhere on the page
                m = self.CASE_NUMBER_RE.search(self.page.inner_text("body"))
                if m:
                    return m.group(0)
            except Exception:
                pass
        ...
        raise AssertionError(
            "EXPECTED: a D[YY]-nnnnnn case number on the LT-261 details page "
            f"({self.page.url}) | ACTUAL: none rendered within {timeout}ms "
            f"(last heading text seen: {last!r})"
        )

    def get_case_number_from_listing(self, vin: str) -> str:
        """Read the FILE NUMBER cell of the listing row holding `vin`."""
        val = self.page.evaluate(
            """(vin) => {
                const rows = [...document.querySelectorAll('table tbody tr')];
                for (const r of rows) {
                    const cells = [...r.querySelectorAll('td')].map(c => (c.innerText||'').trim());
                    if (cells.some(c => c.toUpperCase() === vin.toUpperCase())) {
                        const hit = cells.find(c => /^D\\d{2}-\\d{6}$/.test(c));
                        if (hit) return hit;
                    }
                }
                return null;
        ...
        assert val, (
            f"EXPECTED: a FILE NUMBER (D[YY]-nnnnnn) on the LT-261 listing row for VIN {vin} | "
            f"ACTUAL: no such row/cell on the current listing"
        )
        return val

    def open_details_for_vin(self, vin: str, tab: str = "All", attempts: int = 10) -> str:
        """Listing → `tab` → search `vin` → open the row. Returns the details URL.

        Retries while auto-processing lands the record in the listing.
        """
        for _ in range(attempts):
            try:
                self._dismiss_cdk_overlay()
                t = self.page.locator(f'[role="tab"]:has-text("{tab}")').first
                if t.count():
                    t.click()
                    self._wait_listing_settled()
                self.search_by_vin(vin)
                if vin_row_present(self.page, vin):
                    self.select_application(0)
                    if re.search(r"/LT-261/.+/details", self.page.url, re.I):
                        return self.page.url
            except Exception:
                pass
            self.page.wait_for_timeout(5_000)
        raise AssertionError(
            f"EXPECTED: the LT-261 details page for VIN {vin} to be reachable from the "
            f"'{tab}' listing | ACTUAL: row never resolved after {attempts} attempts"
        )
