"""
E2E-061: Business Registration — Public Portal (End-to-End)

Full registration flow for a new Business/Organization on the NSM Public Portal.

Phases:
  1. [Public Portal]  Click Register → select Business/Organization → fill profile
                      form → keep entered address at the Address Suggestions dialog
                      → agree to Facility Participation → Save
                      → assert "Your profile details have been successfully registered."
  2. [MyNCID]         Sign In with NCID → Create an Account → Business → Continue
...
The five phases share module-level test data and must run in file order — phase N
depends on the account state produced by phase N-1.

All phases drive ONE browser page (the class-scoped `reg_page` fixture) and
...
selected with `pytest --env qa|uat|stage`. Phases 1, 4 and 5 use whatever host that
...
# ─── Shared test data (consistent across all phases) ─────────────────────────
...
TITLE = "Manager"
COMPANY_NAME = f"AutoGarage {_SUFFIX}"
LOCATION_NAME = f"AutoLoc {_SUFFIX}"
...
# the `.env.*` PUBLIC_PORTAL_PASSWORD value). MyNCID's self-registration form
...
def _goto_portal(page: Page):
    """Put the shared page on the portal sign-in screen.
...
class TestE2E061BusinessRegistration:

    # ─────────────────────────────────────────────────────────────────────────
    # Phase 1 — NSM Business profile registration
    # ─────────────────────────────────────────────────────────────────────────

    def test_phase_1_register_business_profile(self, reg_page: Page):
        """
        Go to sign-in page → Register → Business/Organization → fill form →
        ...
        reg = NSMRegistrationPage(reg_page)
        reg.click_register_on_signin_page()
        reg.select_business_organization()

        form = NSMBusinessRegistrationPage(reg_page)
        form.fill_first_name(FIRST_NAME)
        form.fill_last_name(LAST_NAME)
        form.fill_email(EMAIL)
        form.fill_title(TITLE)
        form.fill_company_name(COMPANY_NAME)
        form.fill_location_name(LOCATION_NAME)
        form.fill_address(STREET_ADDRESS)
        form.fill_zip(ZIP_CODE)              # auto-populates city and state
        form.fill_company_email(EMAIL)
        form.fill_primary_contact(PRIMARY_CONTACT)
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # Phase 2 — MyNCID Business account creation
    # ─────────────────────────────────────────────────────────────────────────

    def test_phase_2_create_ncid_account(self, reg_page: Page):
        """
        Sign In with NCID → Create an Account → Business → Continue →
        fill the account form → REGISTER → assert the account-created confirmation.
        """
        print(
        ...
        ncid.select_business_account_type()

        ncid.fill_first_name(FIRST_NAME)
        ncid.fill_last_name(LAST_NAME)
        ncid.fill_username(USERNAME)
        ncid.fill_email(EMAIL)
        # The MyNCID self-registration form is the same for Business and Individual:
        # a Mobile Number (NANP-validated) and — on QA/UAT — a required Confirm
        # Password. There is no confirm-email / address field.
...
def _open_ach_and_add_account(page: Page) -> str:
    ...
    # Business logins can land on a company-selection screen first.
    if re.search(r"select.?company|company.selection|choose", page.url, re.I):
        chooser = _first_visible(
            page,
            page.get_by_role("button", name=re.compile(r"continue|proceed|select", re.I)),
            page.locator("mat-card, .company-card, [class*=company] button").first,
            timeout_ms=5_000,
        )
        if chooser is not None:
            ...
            page.wait_for_timeout(3_000)
            wait_for_portal_ready(page)
        _dump_controls(page, "phase5-after-company-select")
