...
def wait_for_column(page, idx, predicate, tries=40, delay=400):
    ...
    The budget is ~16s because the re-query slows down over a long session: the later
    filters in TEXT_FILTERS run several minutes in, and at the old 6s budget the last one
    (Unique Identifier) could time out against a still-unfiltered grid — surfacing as a
    bogus 'filter returned non-matching rows' failure rather than a timeout."""
...
# TW#27313979 — NCOA Report: Member Number -> UniqueIdentifier (Nordis silent format change).
# TC-27313979-28: the report view must show a real UUID under "Unique Identifier" (not a
# residual "Member Number" label / the old composite-string id), and role access must match
# BR-85 (N&S Admin/User allowed, Fiscal User denied). This suite has no SFTP/DB/Slack/email
# access to verify the other 27 TCs (the backend Nordis import pipeline itself) — see
# nsm-lt260-vinimage-empty-style precedent in test_e2e_033/045/026 for why those are skipped.
UUID_RE = re.compile(r"^[0-9a-fA-F]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-fA-F]{12}$")


@pytest.mark.e2e
@pytest.mark.report
@pytest.mark.medium
class TestTW27313979_NCOAUniqueIdentifier:
    """TC-27313979-28 — Unique Identifier column (no residual 'Member Number'), UUID-shaped
    values, and role-gated access (N&S Admin allowed, Fiscal User denied)."""

    def test_tc28_admin_unique_identifier_column_and_no_member_number_label(self, staff_context: BrowserContext):
        page = staff_context.new_page()
        try:
            go_to_staff_dashboard(page)
            open_ncoa_report(page)

            # Unique Identifier column must be present (the renamed column).
            expect(page.locator('table thead th:has-text("Unique Identifier")').first).to_be_visible(timeout=10_000)

            # No residual "Member Number" label anywhere on the report — the rename must be
            # complete, not a half-migrated mixed state.
            member_number_hits = page.locator('text="Member Number"')
            assert member_number_hits.count() == 0, (
                "Found a residual 'Member Number' label on the NCOA Report — the column "
                "rename to Unique Identifier appears incomplete/mixed"
            )

            # Sample the Unique Identifier column's values and confirm they are UUID-shaped —
            # not the old composite-string id format the ticket's root cause describes.
            uid_col_idx = EXPECTED_COLUMNS.index("Unique Identifier")
            values = nonempty_values(page, uid_col_idx)
            assert values, "No Unique Identifier values found — cannot verify UUID format (see PRE-4: does STAGE have imported NCOA data?)"
            malformed = [v for v in values if not UUID_RE.match(v.strip())]
            assert not malformed, (
                f"Unique Identifier column contains non-UUID-shaped value(s): {malformed[:5]} "
                f"— expected real UUIDs (application_documents.id), not the old composite-string id"
            )
            page.screenshot(path=str(Path(__file__).resolve().parent.parent / "results" / "tw27313979_admin_unique_identifier.png"))
            print(f"  [TC-27313979-28] {len(values)} Unique Identifier value(s) sampled, all UUID-shaped")
        finally:
            page.close()

    def test_tc28_fiscal_user_denied(self, fiscal_context: BrowserContext):
        """Fiscal User sessions redirect straight to the Reports page (no full sidebar —
        see test_e2e_015's established pattern), so this checks the Reports page itself
        for the NCOA Report link rather than routing through the LT-260-sidebar dashboard."""
        ...
        try:
            ...
            ncoa_link = page.locator('//span[contains(text(),"NCOA Report")]').first
            try:
                ncoa_link.wait_for(state="visible", timeout=8_000)
                visible = True
            except PWTimeoutError:
                visible = False
            assert not visible, "Fiscal User should NOT see the NCOA Report link (BR-85 — Nordis reports are N&S Admin/User only)"
            page.screenshot(path=str(Path(__file__).resolve().parent.parent / "results" / "tw27313979_fiscal_denied.png"))
        finally:
            page.close()
