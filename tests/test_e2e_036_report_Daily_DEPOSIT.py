...
Flow:
  ...
  4. Verify Branch column has 'NSS-PrePay' (the branch list always renders). If the report
     carries deposit amounts for today (QA has continuously-seeded data): verify a '$' Total.
     An environment with no deposits for today (e.g. STAGE) shows every branch with a '-'
     Total — steps 5/6 are then skipped, not failed.
...
class TestE2E036DailyDepositReport:
    ...
    def test_phase_1_daily_deposit_report(self, staff_context: BrowserContext):
        ...
        try:
            ...
            # The System Generated Reports table must render before probing a report link.
            # On STAGE that list is slow (cold Angular bundle), and the old flat 10s wait on
            # the link timed out before the table had painted — a slow list read as a missing
            # report. Gate on the table first (generous), then find the link.
            ...
            deposit_link.wait_for(state="visible", timeout=20_000)
            deposit_link.click()
            ...
            # checked against it, not just against a fixed extension/filename. The branch
            # list (incl. 'NSS-PrePay') always renders — asserted above; the money columns
            # only carry values when deposits landed for the date.
            ...
            # Daily Deposit is date-scoped live data. QA carries continuously-seeded
            # deposits, but an environment with no deposits for TODAY renders the full
            # branch list with every money column blank (observed on STAGE 2026-07-23: all
            # branches show '-'). The report and its branch structure are verified above;
            # the '$'-amount check, content-match and downloads need real monetary data, so
            # skip them with an explicit reason rather than asserting QA-only deposits.
            has_money = any(re.search(r"\d", t or "") for t in ui_branch_totals.values())
            if not has_money:
                pytest.skip(
                    f"Daily Deposit Report has no deposit amounts for {TODAY_MMDDYYYY} in "
                    f"env '{ENV_NAME}' — heading/columns/branches verified; amount + download "
                    f"checks skipped (no deposits to assert against)"
                )

            # Total column has at least one '$' value now that we know deposits exist.
