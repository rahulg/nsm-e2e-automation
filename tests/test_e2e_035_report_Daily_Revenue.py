...
Flow:
  ...
  4. If the report returned revenue rows (QA has continuously-seeded data): verify a real
     FEE CODE row with a '$' AMOUNT. An environment with no payments for today (e.g. STAGE)
     legitimately returns only a 'Total' summary row — steps 5/6 are then skipped, not failed.
...
class TestE2E035DailyRevenueReport:
    ...
    def test_phase_1_daily_revenue_report(self, staff_context: BrowserContext):
        ...
        try:
            ...
            # checked against it, not just against a fixed extension/filename. The report
            # always renders a trailing 'Total' summary row; real revenue lines are all
            # the others.
            ui_headers, ui_rows = scrape_report_table(page)
            ui_fee_amounts = extract_fee_amounts(ui_headers, ui_rows)
            revenue_rows = {fc: amt for fc, amt in ui_fee_amounts.items()
                            if fc.strip().lower() != "total"}
            print(f"\n  [E2E-035] UI fee code/amount rows: {ui_fee_amounts}")

            # Daily Revenue is date-scoped live data. QA carries continuously-seeded
            # transactions, but an environment with no payments for TODAY legitimately
            # returns no revenue lines (observed on STAGE 2026-07-23: only a 'Total  -'
            # row). The report and its structure are already verified above; the
            # content-match and download checks below need real rows, so skip them with an
            # explicit reason rather than asserting a QA-only seed value like '00007NOIC'.
            if not revenue_rows:
                pytest.skip(
                    f"Daily Revenue Report returned no revenue rows for {TODAY_MMDDYYYY} in "
                    f"env '{ENV_NAME}' — heading/columns verified; data + download checks "
                    f"skipped (no daily revenue to assert against)"
                )

            # AMOUNT column has at least one '$' value on a real revenue row.
