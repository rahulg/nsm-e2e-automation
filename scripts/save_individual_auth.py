...
def wait_for_session(page, timeout: int = 60_000) -> None:
    ...
    Same fix as save_public_auth.py's wait_for_session — the NCID SSO hand-back lands on
    '/authentication/validate?code=...' BEFORE the app has exchanged that code for a token.
    Saving there produces a storage_state whose nsm-qa-public localStorage holds only
    'path'/'loginTime' — no authToken/refreshToken/userData — so every later run bounces
    straight back to /ncdot-nsm-signin. Wait for the dashboard route AND for authToken to
    be written, and fail loudly rather than persist a useless state file.
    """
    try:
    ...
    try:
        ...
        raise RuntimeError(
            f"Individual-portal login did not produce an authToken in localStorage "
