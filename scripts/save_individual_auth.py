"""Generate auth/individual-portal.json by logging in as the individual public user via NCID."""
import os
import re
import sys
...
AUTH_PATH = Path(__file__).resolve().parent.parent / "auth" / "individual-portal.json"
...
def main():
    with sync_playwright() as p:
        ...
        # Enter username
        page.locator("#identifierInput").fill(ENV.INDIVIDUAL_PUBLIC_USERNAME)
        page.locator('a.ping-button:has-text("Next")').click()
        page.wait_for_load_state("networkidle")

        # Enter password
        page.locator("#password").fill(ENV.INDIVIDUAL_PUBLIC_PASSWORD)
